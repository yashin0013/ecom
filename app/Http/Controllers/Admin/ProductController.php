...
class ProductController extends BaseController
{
    ...
    public function bulk_import_data(Request $request)
    {
       ...
       ->where('name',  $collection['category_name'])
       ->get();
    //   echo var_dump($catIds);
             $lists =[];
            foreach ($catIds as $list) {
                $lists = $list->id;
                 
             }
             
             $subcatIds = DB::table('categories')
               ->where('name',  $collection['sub_category_name'])
               ->where('parent_id',  $lists)
               ->get();
    
             $rows =[];
            foreach ($subcatIds as $row) {
                $rows = $row->id;
             }
             
              $brandIds = DB::table('brands')
               ->where('name',  $collection['brand_name'])
               ->get();
            ...
            array_push($data, [
                ...
                'category_ids' => json_encode([['id' => $lists, 'position' => 0], ['id' => $rows, 'position' => 1]]),
                'brand_id' => $brands,
